...
| `src/middleware.ts` | Edge-runtime route protection (refresh-token-cookie presence check); `/feedback` and `/loyalty` are exempted from the staff gate entirely (see Risks — this fixed a pre-existing bug for `/feedback`). `/forgot-password` and `/reset-password` are public too, but deliberately **not** in `STAFF_AUTH_PATHS`: that list also drives the "already signed in → bounce to `/dashboard`" rule, which would strand a user who is resetting *because* they suspect a compromise and is still signed in on the device reading the email |
...
| `src/app/forgot-password/`, `src/app/reset-password/` | Staff account recovery (Block 1.6). `/forgot-password` is a client form that always renders the same hedged confirmation, whether or not the address matched an account — the browser-side half of the API's enumeration defence. `/reset-password` is a Server Component that reads `?token=` and hands it to `reset-password-form.tsx`, rather than a client component calling `useSearchParams()` |
...
| Concurrent `apiFetch()` calls could rotate the same refresh token | Fixed | `api-client.ts` shares one in-flight refresh request per refresh token. Every waiting caller retries with the same new access token and writes the same rotated cookie pair, so a losing request cannot clear the winner's valid session. |
...
| Suspended/deactivated users keep using a still-valid access token on ordinary business routes | Resolved | `authenticate` now reloads the token subject on every request and requires an active, non-deleted user. Impersonation tokens also revalidate the platform actor, so deactivating either identity immediately ends access instead of waiting for JWT expiry. `requirePlatformRole` retains its own status check as defense in depth. |
