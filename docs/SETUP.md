...
| `ANTHROPIC_DAILY_SPEND_LIMIT_USD` | self (S4.2 spend-limit guardrail) | Required | api runtime | `10` | plain decimal string, parsed with `Number()`; SummaryService blocks (and logs to `ai_usage_log`) further summary generation for the rest of the UTC day once reached | No |
| `ANTHROPIC_MONTHLY_SPEND_LIMIT_USD` | self (S4.2 spend-limit guardrail) | Required | api runtime | `150` | plain decimal string, parsed with `Number()`; same blocking behavior as the daily limit, for the rest of the UTC calendar month | No |
...
| `WEB_BASE_URL` | self (the deployed `apps/web` origin) | Required | api runtime | `https://app.example.com` | single origin, no trailing slash needed; must serve `/reset-password` — this is the base of the link inside password-reset emails, so a wrong value sends users somewhere that cannot complete recovery | No |
